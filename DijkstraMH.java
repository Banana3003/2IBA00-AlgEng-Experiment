...
public class DijkstraMH {
    ...
    public DijkstraMH (Graph graph, int src) {
        this.graph = graph;
        this.src = src;
        ...
        pqueue = new MinHeap(graph.nodeCount);
    }

    public void run() {
        // Put all verticies in min-priority queue with d(v) as key
        for (int i = 0; i < graph.nodeCount; i++) {
            pqueue.insert(new HeapNode(this.dist[i], i));
        }
    ...
    public static void main(String[] args) {
        ...
        DijkstraMH dijkstra = new DijkstraMH(graph, 0);
